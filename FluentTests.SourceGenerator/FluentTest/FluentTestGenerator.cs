...
public class FluentTestGenerator : ISourceGenerator
{
    ...
    {
        if (_context.SyntaxReceiver is not FluentTestSyntaxReceiver receiver) return;
        _context = context;
    ...
    private List<INamedTypeSymbol> GetClassSymbolsWithTheRightAttribute(FluentTestSyntaxReceiver receiver)
    {
        ...
        var attributeSymbol =
            compilation.GetTypeByMetadataName(typeof(FluentTestAttribute).Namespace + nameof(FluentTestAttribute));
